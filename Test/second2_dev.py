...
def list_ended2(list_to_test, list_index):
...
def get_response2(full_list, list_index):
    
    response_obj = response_interface_pack([],list_index)
    
    if list_ended2(full_list, list_index):
        # list has ended
        response_obj.responses.append('cap_file_ended')
        return response_obj
    ...
    if not is_a_response(line):
        ...
        response_obj.responses.append('no_response_-_the_following_communication_is_also_a_request!')
        return response_obj
    
    
    # found a response
    response_obj.responses.append(line[2])
    ...
    while resp_pending and list_not_ended:
        ...
        if list_ended2(full_list, list_index):
            ...
            if res_or_req == '3E.01.': 
                ...
                response_obj.responses.append(res_or_req)
    ...
    response_obj.list_index = list_index
...
def get_response(full_list, list_index):
    ...
    if list_ended(list_lenght, list_index):
        # list has ended
        responses.append('cap_file_ended')
        full_response =  'cap_file_ended'
        ...
        if is_a_response(line):
            # found a response
            response_first_part = line[2]
            
            # skipping negative pending response and tester present communication
            res_or_req = line[2]
            resp_pending = False
            
            if response_first_part[0:2] == '7F': 
                # negative response
                if response_first_part[-3:-1] == '78':
                    # response pending
                    resp_pending = True
            
                while (resp_pending) and (not list_ended(list_lenght, list_index)):
                # case that response have to wait (response 7F.XY.78 - 78 means response is pending) and skip its tester present communication    
                    list_index += 1 # go to next line
                    
                    if list_ended(list_lenght, list_index):
                        response_second_part =  'cap_file_ended'
                        pass
                        ...
                        if res_or_req == '3E.01.': 
                            ...
                            print('Pending response found')
                            response_second_part = res_or_req
            ...
            if response_second_part == '':   
                responses.append(response_first_part)
                full_response = response_first_part
                list_index += 1
            else:
                responses.append(response_first_part)
                responses.append(response_second_part)
                full_response = response_first_part + ',' + response_second_part   
                 
        else:
            responses.append('no_response_-_the_following_communication_is_also_a_request!')
            full_response = 'no_response_-_the_following_communication_is_also_a_request!'
    
    return full_response, list_index
...
def req_and_res_2(full_list):
    ...
    while not list_ended(list_lenght, list_index):
        ...
        if is_a_request(line):
            ...
            responses, list_index = get_response(full_list, list_index)
                
            request_response_object.responses.append(responses)
...
def get_new_services(file_name_after_framesCAN):
    ...
    if list_req_and_res == 0:
        ...
        for i in lines:
            ...
            line_split = i.split(" ")
            request = line_split[1]
            responses = line_split[3:]
            
            req_res_obj = request_responses(request, responses)
            req_resp_object_list.append(req_res_obj)
            req_res_obj.print_req_resp()
...
def req_resp_list(file_name_after_framesCAN):
